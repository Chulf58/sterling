...
6. **Capture.** Anti-pattern (what broke / wrong way / right way — noise-gated, evidence-required); disconfirmed hypotheses from the fan-out's refutations (§3.2.8); article reconciliation; regression test promoted; workaround removed if one was built; boarded task removed. H3 debug-scope mode live throughout (the explorer map registers as the lightweight contract).
