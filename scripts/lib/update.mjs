...
export function refusalFor(c) {
  ...
  if (c.dirty_tracked.length) {
    return (
      'update: uncommitted changes to tracked files — a consumer machine has nothing to merge, so this is either accidental drift or work that belongs on the authoring machine. Discard (git checkout -- .) or commit and push from the authoring machine, then rerun:\n' +
      c.dirty_tracked.map((l) => `  ${l}`).join('\n')
    );
  }
  if (c.ahead > 0) {
    return c.behind > 0
