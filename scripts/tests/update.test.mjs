...
// architecture.md in the way, a symlink, an ignore rule) must not be buried under
// a completion marker: the update exits non-zero, writes no marker, and the next
// /sterling:update (even with nothing to pull) resumes and retries the projection.
test('fan-out: an actionable handoff conflict withholds the completion marker, and the next update retries it', async () => {
  const cwd = scratchCwd();
  try {
    const runOnce = async (behind, handoffStatus) => {
      const { exec: base, calls } = fakeExec({ behind, changed: ['packages/store/src/index.ts'] });
      const exec = (cmd, args, o) => {
        if (!args[0]?.endsWith('handoff-projection.mjs')) return base(cmd, args, o);
        calls.push(`${cmd} ${args.join(' ')}`);
        const stdout = handoffStatus === 3
        ...
        return { status: handoffStatus, stdout, stderr: '' };
      };
      const lines = [];
      const report = await runUpdate({ cwd, exec, log: (l) => lines.push(l), projects: [{ name: 'target', repo_path: '/tmp/handoff-target' }], opts: {} });
      return { report, calls, log: lines.join('\n') };
    };

    const first = await runOnce(2, 3);
    assert.notEqual(first.report.exit, 0, 'an actionable conflict fails the update');
    assert.match(first.log, /✗ handoff projection: REFUSED — architecture\.md exists/);
    assert.ok(!existsSync(join(cwd, UPDATE_MARKER_RELATIVE_PATH)), 'no completion marker while the conflict stands');

    const second = await runOnce(0, 0);
    assert.equal(second.calls.filter((c) => c.includes('handoff-projection.mjs')).length, 1, 'the next update resumes and retries the projection');
    assert.equal(second.report.exit, 0);
    assert.ok(existsSync(join(cwd, UPDATE_MARKER_RELATIVE_PATH)), 'the marker is written once the projection succeeds');
  } finally {
    rmSync(cwd, { recursive: true, force: true });
  }
});
