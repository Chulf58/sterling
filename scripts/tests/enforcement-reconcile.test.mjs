// H17 (B) SURFACE — THE RECONCILIATION CLEARER (Sterling SLICE S4r).
//
// Spec: board `13d5f6e3-ca1d-431d-8c11-7ebd37bf6780` ("S4r — THE GLOBAL LATCH
// GATE + THE SEPARATE RECONCILIATION CLEARER"), PART B specifically. Refined by
// decision `h17-global-registration-is-gated-on-a-live-subagent-delivery-probe-and-the-clearer-ships-first`
// (b3cfdbc5) Rulings R2 and R3 (the clearer ships FIRST, before global
// registration), and decision
// `b-surface-adoption-point-closes-with-an-incident-bound-taint-latch-not-a-persisted-manifest`
// (bcd2cc09) Ruling 5 (split attestation from clearance — the producer never
// clears) and Ruling 10 (stamp first, clear last; the latch always wins).
//
...
// path, the stamp path/shape, the (B) path set) to construct fixtures the
// clearer must react to correctly.
//
// ===========================================================================
...
// CHECKED CLEARANCE, not a boundary: the surface is proven to MATCH ITS STAMP AT
...
// to scripts/hooks/h17-bash-write-sweep.mjs / scripts/enforcement-stamp.mjs,
// per H4 — no hook or CLI source was read to write these pins. Every
// expectation comes from the board item and decision prose above (a decision
// record and a board item are spec, not implementation — H4's wall gates
// Read/Grep on code, not knowledge_get/board_get).
//
// ===========================================================================
// THE ENTRY POINT — UNDECIDED IN THE BRIEF, SO THIS FILE DECIDES IT AND THE
// IMPLEMENTER BINDS TO IT (the board prefers a conductor-only MCP operation
...
//                                   // once, after the stamp has been read and
//                                   // the current (B) set has been
//                                   // enumerated+hashed and a verification
//                                   // verdict computed, but strictly BEFORE
//                                   // any mutation (latch removal) is
//                                   // attempted. Production callers (the MCP
//                                   // tool) MUST NEVER pass this. It exists
//                                   // solely so this file can inject a
//                                   // deterministic TOCTOU race instead of
//                                   // guessing at timing.
...
//     }) => Promise<{
//       cleared: boolean,           // true iff the latch was removed THIS call
//       reason: string,             // human-readable; never thrown away
//     }>
//
// ===========================================================================
...
//   security rests on this one argument, because a stamp matching current disk
...
//     the last re-read of the stamp, the last re-enumeration/re-hash of the
...
//   LATCH's own identity. Nothing binds the STAMP or the (B) SURFACE across
//   that interval, so an attacker who wins the window can substitute a
//   TAMPERED surface together with a MATCHING attacker stamp — self-consistent,
...
//   (e.g. carry the stamp's own content hash + each (B) leaf's dev/ino/size/
...
//   set matches the stamp exactly. AC-R20 pins the same guard's
...
//   (c) directory-membership re-listing, or (d) the stamp's own bound
...
//     - A LEAF/MEMBER/STAMP tamper injected here is past the root/edge check
...
//   changed mid-flight  "the (B) surface / stamp CHANGED DURING VERIFICATION"
//                                                        [NOT plain mismatch]
//   stamp absent        "no stamp present"
//   stamp malformed     "the stamp is malformed / unparseable JSON"
//   stamp incomplete    "no stamp entry for <path> (unattested)"
//   stamp hash mismatch "hash mismatch for <path>"
//   stamp duplicate     "duplicate stamp entry for <path>"
//   stamp stale entry   "<path> is attested by the stamp but no longer exists"
//   deleted-but-present "<path> is attested deleted but is present on disk"
...
//                        agent-caller wording, NOT any stamp family]
...
//                        appeared)"                                    — (c)
//   stamp rebound       "the stamp changed after the verdict"          — (d)
...
// refusal path (stamp absent/malformed/incomplete/duplicated/non-exact/
...
// oneLine/stampPath/writeStamp/coderPath/sha256Of/latchPath idiom from
...
const REASON = {
  ...
  incomplete: /incomplete|unattested|not attested|no stamp entry|missing (a )?stamp entry/i,
  hashMismatch: /hash mismatch|hash does not match|bytes do not match|differs from the stamp/i,
  duplicate: /duplicat|more than one (stamp )?entry|repeated entry/i,
  staleEntry: /no longer exists|not present on disk|stale (stamp )?entry|attested .* (is )?absent|missing from the current/i,
  deletedButPresent: /deleted/i,
  ...
  // it detected a CHANGE, only that the surface disagrees with the stamp.
  ...
  // The sibling AC-R8/AC-R9/AC-R19 must NOT resolve to.
  ordinaryMismatch: /\bmismatch\b|does not match the stamp|\bunattested\b|\bincomplete\b|\bduplicate\b/i,
...
//   * every positive-path pin (AC-R1, AC-R22, AC-R25, AC-R35, AC-R40) asserts
//     `cleared:true` against a call that correctly refuses — three red tests
//     that are the SUITE being wrong, not the module;
//   * every refusal pin asserts a SPECIFIC reason family (stamp, latch shape,
...
// RULING ITSELF there (AC-R40, which runs its natural arm with no seam at all
// on a real Windows host). Deleting the positive-path arms instead would trade
// a red suite for a blind one.
//
...
// Windows host the suite returns to red-failing AC-R1/AC-R22/AC-R25/AC-R35 and
...
const WIN32_SKIP = ON_WIN32
  ? 'win32: decision 0ac7be95 R4 rules that reconciliation REFUSES before any I/O on this platform, so no verification-path or removal-path behaviour is observable here. AC-R40 pins that ruling natively on this host.'
  : false;
...
// The exact stamp for the (B) surface makeGitProject() produces — what a
// correctly-run producer (scripts/enforcement-stamp.mjs) would have written.
// Fabricated directly rather than by invoking the producer script: this file
// has no verified knowledge of that script's CLI contract (H4 forbids reading
// it to find out), and the sibling S3 latch suite already establishes
// fabricating the stamp file directly as the accepted testing idiom for
// exercising stamp-shaped inputs.
function exactStampFor(dir) {
  return [
...
function stampEntryFor(dir, rel) {
  return { path: rel, sha256: sha256Of(absOf(dir, rel)), at: NOW };
}

// The exact stamp for the base fixture PLUS a set of extra (B) paths, each
...
function exactStampForWith(dir, extraRels) {
  return [...exactStampFor(dir), ...extraRels.map((rel) => stampEntryFor(dir, rel))];
}

// Writes the stamp file VERBATIM (no JSON.stringify), for fixtures whose point
// is the raw byte length rather than the parsed content. AC-R44 uses it.
function writeRawStamp(dir, text) {
  const p = stampPath(dir);
  mkdirSync(dirname(p), { recursive: true });
...
// Stamp entries for an arbitrary list of repo-relative (B) paths, hashed from
// current disk. Companion to exactStampFor(), which covers only the three
// paths makeGitProject() creates.
function stampEntriesFor(dir, rels) {
  return rels.map((rel) => ({ path: rel, sha256: sha256Of(join(dir, ...rel.split('/'))), at: NOW }));
}
...
const BINDING = {
  ...
  // (d): the STAMP's own binding, as distinct from any (B) leaf's.
  stampBinding: /stamp/i,
...
// "producer cannot clear"): a stamp appearing on disk — however it got there —
...
// stamp appears, BEFORE reconcileEnforcementTaint is ever called — is the
...
// ===========================================================================
test('AC-R1 CONTROL: fabricating a valid stamp does NOT by itself clear the latch; only the explicit reconcile call does, and it succeeds on an exact match', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    assert.equal(existsSync(latchPath(dir)), true, 'PRECONDITION: latch is present');

    writeStamp(dir, exactStampFor(dir));
    assert.equal(
      existsSync(latchPath(dir)),
      true,
      'THE LAUNDERING PIN: a stamp appearing on disk (by any means — this is exactly what a background child stamping tampered bytes would produce) never by itself clears the latch; stamping is not clearing'
    );

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, true, `an exact stamp for the current (B) surface must clear — reason: ${reasonOf(result)}`);
...
// ===========================================================================
// AC-R2 — STAMP ABSENT REFUSES, EVEN WHEN THE (B) SURFACE IS ALREADY
// SELF-CONSISTENT (matches its as-committed HEAD state, nothing tampered).
// This is deliberately the sharpest form of "a stamp that exactly equals the
// current (B) surface is NOT sufficient on its own" (task item: a stamp must
// be independently trusted, not merely self-consistent) — here there is no
// stamp file AT ALL, and the surface looks perfectly fine, and it must STILL
...
// actually consulting the stamp path rather than re-deriving trust from
// current disk state.
//
// SABOTAGE: treat a missing stamp as "nothing to verify against, so trust
// current disk state" and clear anyway — `result.cleared` flips true, and
// `existsSync(latchPath(dir))` flips false.
// ===========================================================================
test('AC-R2: stamp file ABSENT refuses, even though the (B) surface is currently self-consistent (never touched)', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    ...
    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, false, `an absent stamp must refuse to clear — reason: ${oneLine(result.reason)}`);
    assert.match(oneLine(result.reason), /stamp/i, 'the refusal reason names the stamp');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives an absent-stamp refusal');
  } finally {
    cleanup();
  }
});
...
// ===========================================================================
// AC-R3 — STAMP MALFORMED JSON refuses.
//
// SABOTAGE: wrap the stamp read+parse in `try { ... } catch { stamp = [] }` —
// the empty manifest then refuses down the INCOMPLETE path instead, so
// `cleared:false` still holds and only the malformed-family reason assertion
// fires. (That substitution is precisely the hollowness class this repair pass
// exists to close.)
// ===========================================================================
test('AC-R3: a malformed (unparseable JSON) stamp refuses, latch survives', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    mkdirSync(dirname(stampPath(dir)), { recursive: true });
    writeFileSync(stampPath(dir), '{ not valid json,,,');

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, false, `a malformed stamp must refuse — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), REASON.malformed, `the refusal is attributed to the MALFORMED-stamp guard, not to some sibling refusal — reason: ${reasonOf(result)}`);
    assert.doesNotMatch(reasonOf(result), REASON.noLatch, 'a malformed stamp is not a no-latch no-op');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a malformed-stamp refusal');
  } finally {
    cleanup();
  }
});
...
// ===========================================================================
// AC-R4 — STAMP INCOMPLETE: it attests only two of the three current (B)
// paths, silently omitting one that currently exists on disk. Exact-SET
// equality means a stamped subset is not enough.
//
// SABOTAGE: validate only the entries the stamp DOES contain (subset
// comparison) instead of requiring every current (B) path to have an entry —
// `result.cleared` flips true.
// ===========================================================================
test('AC-R4: a stamp that omits a currently-existing (B) path refuses (subset is not exact)', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    const full = exactStampFor(dir);
    writeStamp(dir, full.filter((e) => e.path !== SETTINGS_REL)); // omit settings.local.json

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, false, `an incomplete stamp must refuse — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), REASON.incomplete, `the refusal is attributed to the INCOMPLETE-stamp guard — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), /settings\.local\.json/, 'the refusal names the (B) path that has no stamp entry — an unattributable "something did not match" is not a usable verdict');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives an incomplete-stamp refusal');
  } finally {
    cleanup();
  }
});
...
// ===========================================================================
// AC-R5 — STAMP HASH MISMATCH: an entry names a current (B) path but with a
// hash that does not match its actual current bytes.
//
// SABOTAGE: compare only PATH PRESENCE, not the hash value — `result.cleared`
// flips true.
// ===========================================================================
test('AC-R5: a stamp entry whose hash does not match its current (B) path bytes refuses', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    ...
    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, false, `a hash-mismatched stamp entry must refuse — reason: ${reasonOf(result)}`);
...
// ===========================================================================
// AC-R6 — STAMP DUPLICATE ENTRIES for the same path (even with a CORRECT
// hash on both) refuses — duplication itself makes the manifest ambiguous,
// isolated here from "wrong hash" by giving the duplicate the SAME correct
// hash as the original entry.
//
// SABOTAGE: use `Array.find()` (first match) instead of validating the
// manifest has exactly one entry per path — `result.cleared` flips true,
// because `find()` would silently accept the first (correct) entry and never
// notice the duplicate.
// ===========================================================================
test('AC-R6: a stamp with a DUPLICATE entry for the same path refuses, even when the duplicate has the same correct hash', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    const full = exactStampFor(dir);
    const coderEntry = full.find((e) => e.path === CODER_REL);
    writeStamp(dir, [...full, { ...coderEntry }]); // exact duplicate, not a conflicting one

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, false, `a duplicated stamp entry must refuse even with a matching hash — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), REASON.duplicate, `the refusal is attributed to the DUPLICATE guard — with a matching hash, any other family would mean the duplicate itself went unnoticed — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), /coder\.md/, 'the refusal names the duplicated path');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a duplicate-entry refusal');
  } finally {
    cleanup();
  }
});
...
// ===========================================================================
// AC-R7 — STAMP LISTS A PATH THAT NO LONGER EXISTS in the current (B) set
// (attested, then deleted before reconciliation was attempted). Exact-SET
// equality runs both directions: the stamp must not claim more than the
// current surface has, just as AC-R4 shows it must not claim less.
//
// CONTRAST PAIR — read this with AC-R22. The entry here attests the path as
// PRESENT (it carries a sha256) and the path is now gone: that is a STALE
// entry and it refuses. An entry that attests `deleted:true` for a path that
// is still absent is a LEGITIMATE attestation and CLEARS (AC-R22, conductor
// ruling). The two must not collapse into one another.
//
// SABOTAGE: only check "every current path has a matching stamp entry"
// (AC-R4's direction) without also checking "every stamp entry has a
// currently-existing path" — `result.cleared` flips true.
// ===========================================================================
test('AC-R7: a stamp entry for a (B) path that no longer exists on disk refuses (exactness runs both directions)', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    ...
    writeStamp(dir, [...full, { path: '.claude/agents/removed-agent.md', sha256: extraHash, at: NOW }]);
    rmSync(extraPath); // now the stamp claims a path the current (B) set does not have

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, false, `a stamp entry with no corresponding current (B) path must refuse — reason: ${reasonOf(result)}`);
    ...
    assert.match(reasonOf(result), /removed-agent\.md/, 'the refusal names the stamp entry that no longer has a path');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a stale-stamp-entry refusal');
  } finally {
    cleanup();
  }
});
...
// (stamp read + (B) enumeration/hash + verdict) and the removal. Uses the
...
test('AC-R8: the (B) surface mutating between verification and removal aborts the clear — latch survives', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // exact at the moment reconcile starts
...
// ===========================================================================
// AC-R9 — TOCTOU: the STAMP mutates between verification and removal.
//
// SABOTAGE: read the stamp once at the start and never re-check it before the
// irreversible removal — `existsSync(latchPath(dir))` flips from true to
// false.
// SECOND SABOTAGE (aimed at the new discriminator): re-read the stamp but
// report the disagreement as an ordinary hash mismatch — the
// changed-during-verification assertion fires. See AC-R8's repair note.
// ===========================================================================
test('AC-R9: the stamp file mutating between verification and removal aborts the clear — latch survives', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir));
    ...
    const result = await reconcileEnforcementTaint({
      ...
      ...CONDUCTOR,
      _testHookAfterEnumeration: async () => {
        // Corrupt the stamp AFTER the (correct) verdict was computed against it.
        writeFileSync(stampPath(dir), JSON.stringify([{ path: CODER_REL, sha256: 'deadbeef', at: NOW }]));
      },
    });
    assert.equal(result.cleared, false, `a stamp mutation during verification must abort the clear — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), REASON.changed, `THE DISCRIMINATOR: the verdict must be attributed to the STAMP CHANGING DURING VERIFICATION — reason: ${reasonOf(result)}`);
    assert.doesNotMatch(reasonOf(result), REASON.ordinaryMismatch, 'a plain manifest-mismatch reason cannot distinguish this from an ordinary bad stamp (AC-R4/AC-R5)');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a mid-flight stamp mutation');
  } finally {
    cleanup();
  }
});
...
// fixture (exact stamp, real latch) except for `callerAgentId` — isolating
...
// named at the top of this file: even a child that reproduces a valid stamp
...
test('AC-R10: a caller presenting an agent identity is refused even against AC-R1\'s exact-match success fixture — conductor-only', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // identical to AC-R1's proven-success fixture
...
// create a stamp, a latch, or otherwise leave a side effect.
//
...
test('AC-R12: with no latch present, reconciliation is a no-op — never falsely claims a clear happened', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampFor(dir)); // even with a perfectly valid stamp present
...
// an otherwise-exact stamp (isolating shape as the sole variable from AC-R1's
...
// known content elsewhere in the project. Otherwise-exact stamp, isolating
...
// latch path, otherwise-exact stamp. "An unreadable latch is not an absent
...
test('AC-R16: a symlink swapped in at the latch path between verification and removal aborts — victim file untouched, no false success', { skip: BEHAVIOURAL_SKIP || SYMLINK_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampFor(dir)); // exact — verification alone would succeed (AC-R1)
...
// longer agrees with the (untouched) stamp. Its diagnostic value is entirely
...
test('AC-R18: a (B) surface tamper AFTER the final verdict and BEFORE the removal walk must not clear', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // exact — AC-R1 proves this fixture otherwise clears
...
//
// Found by outside-model review; provably invisible to AC-R8/AC-R9, which is
// why it needs its own seam: the module re-reads the stamp, re-enumerates the
...
// Nothing binds the stamp or the surface across that interval, and
...
// ATTACKER STAMP: the pair is self-consistent, the latch is untouched and
// still identical, the unlink proceeds, and the call returns cleared:true.
// Call N+1 then sees no latch and a surface that agrees with its own stamp.
...
// to the exact bytes and identities it was computed from — carry the stamp's
...
// a plain repeat of the verification (re-read stamp, re-hash surface, compare)
// — AC-R18 stays green, THIS test flips to cleared:true. That divergence is
// the whole reason both tests exist.
// ===========================================================================
test('AC-R19 CRITICAL: a post-verdict tamper accompanied by a MATCHING attacker stamp (self-consistent) must not clear — the verdict must be bound to the bytes it was computed from', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // good stamp + good surface for BOTH verification passes
    ...
    const result = await reconcileEnforcementTaint({
      ...
      _testHookBeforeRemoval: async () => {
        hookCalls += 1;
        // The exploit, in two lines: tamper the surface, then stamp the
    ...
    assert.doesNotMatch(
      ...
      'there is nothing to "mismatch" here — the surface and the stamp agree perfectly. A mismatch reason would mean the refusal came from somewhere other than the identity binding this test names'
    );
...
// exactly the three attested paths, the stamp matches EXACTLY, and the latch
// is discharged while a tampered grant sits on disk inside it.
//
// ISOLATION: the stamp here is byte-identical to AC-R1's proven-success
// stamp, and the only difference from AC-R1's fixture is the unenumerable
...
// [] }` — the hidden file becomes invisible, the set matches the stamp, and
// `result.cleared` flips true.
// ===========================================================================
test('AC-R20: a (B) subdirectory that cannot be enumerated REFUSES — it is never silently treated as empty', { skip: BEHAVIOURAL_SKIP || UNREADABLE_DIR_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampFor(dir)); // attests ONLY the three visible (B) paths
...
// enumerated normally, and the call refuses as an INCOMPLETE stamp — the
...
// vanished directory's contents disappear from the set, the stamp matches
...
// ===========================================================================
// AC-R22 — `deleted:true` FOR A STILL-ABSENT PATH IS A LEGITIMATE
// ATTESTATION AND CLEARS. CONDUCTOR RULING (S4r adjudication), recorded here
// because the behaviour is currently ACCIDENTAL — untested in either
// direction, so nothing stops a future change from inverting it silently.
//
// An outside review flagged this as LOW, arguing it contradicts "a stale entry
// for a deleted path must refuse". IT DOES NOT, and the distinction is the
// point of the AC-R7 CONTRAST PAIR: AC-R7's entry attests the path as PRESENT
// (it carries a sha256) and the path is gone — that is a STALE entry and it
// refuses. THIS entry attests that the producer OBSERVED THE DELETION, and the
// path is still absent — the attestation and the disk agree, so the surface is
// fully accounted for. The ruling was NOT to invert it.
//
// PLACED BEFORE AC-R23 as its control: it must pass for the OPPOSITE reason.
// Without it, AC-R23's refusal could equally mean "any entry carrying
// `deleted` is rejected outright", which would pin nothing about presence.
//
// SABOTAGE: reject any stamp entry carrying `deleted:true` (or require a
// sha256 on every entry) — `result.cleared` flips false.
// ===========================================================================
test('AC-R22 CONTROL: a stamp entry attesting deleted:true for a path that is STILL ABSENT clears — a deletion attestation is not a stale entry', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    writeStamp(dir, [...exactStampFor(dir), { path: '.claude/agents/removed-agent.md', deleted: true, at: NOW }]);
    assert.equal(existsSync(join(dir, '.claude', 'agents', 'removed-agent.md')), false, 'PRECONDITION: the attested-deleted path really is absent');

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, true, `THE RULING: a deletion attestation for a still-absent path is accounted-for surface, not a stale entry — reason: ${reasonOf(result)}`);
    assert.equal(existsSync(latchPath(dir)), false, 'the latch is removed');
  } finally {
    cleanup();
  }
});
...
// ===========================================================================
// AC-R23 — `deleted:true` FOR A PATH THAT IS PRESENT AGAIN REFUSES. The other
// half of the ruling, and the half that carries the security weight: an entry
// claiming "this file is gone" while the file sits on disk attests nothing
// about the bytes that are actually there, so the surface is unaccounted for.
// Resurrecting a deleted agent definition after stamping its deletion must not
// be a clearable state.
//
// The fixture is IDENTICAL to AC-R22's — proven to clear — except that the
// attested-deleted path exists again. That isolation is what makes the
// refusal attributable to presence rather than to `deleted` handling in
// general.
//
// EXPECTED RED (reason only) against the current implementation: it very
// likely already refuses, but ACCIDENTALLY — via the entry having no sha256 to
// compare — which is an unattributable verdict. The reason assertions force a
// deliberate branch.
//
// SABOTAGE: skip presence checking for entries carrying `deleted:true`
// (short-circuit the entry as satisfied) — `result.cleared` flips true.
// ===========================================================================
test('AC-R23: a stamp entry attesting deleted:true for a path that is PRESENT AGAIN refuses', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    writeStamp(dir, [...exactStampFor(dir), { path: '.claude/agents/removed-agent.md', deleted: true, at: NOW }]);
    // The single variable vs AC-R22: the attested-deleted path is back.
    writeFileSync(join(dir, '.claude', 'agents', 'removed-agent.md'), '# resurrected after its deletion was attested\n');

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.equal(result.cleared, false, `a path attested as deleted but present on disk must refuse — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), REASON.deletedButPresent, `the verdict must be attributed to the DELETION ATTESTATION being contradicted, not to some incidental missing hash — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), /removed-agent\.md/, 'the refusal names the resurrected path');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives');
  } finally {
    cleanup();
  }
});
...
// protocol offers no second line of defence, because a stamp matching current
// disk is otherwise sufficient to clear.
//
// ISOLATION AND CONTROL: the fixture is byte-identical to AC-R1's
// proven-success fixture — exact stamp, real latch — with the conductor
...
// "an agent presented itself", and must not read as any stamp-family refusal
// (there is nothing wrong with the stamp).
//
...
test('AC-R24: an OMITTED conductor assertion REFUSES — absence is not evidence of conductor (fail-closed identity gate)', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  // shape).
  {
    ...
    try {
      plantLatch(dir);
      writeStamp(dir, exactStampFor(dir)); // AC-R1's exact proven-success fixture
      ...
      assert.doesNotMatch(reasonOf(result), REASON.stamp, 'the stamp is exact here; a stamp-family reason would mean this refusal came from somewhere other than the identity gate');
...
//     (AC-R46) and the stamp's IDENTITY arm (AC-R47).
...
test('AC-R25 CONTROL: SEAM 4 and SEAM 4b each fire exactly once, in that order, with the bound-confirmation callback, and no-op hooks still CLEAR', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // AC-R1's proven-success fixture, untouched
...
test('AC-R26: a (B) leaf RENAME-SUBSTITUTED (byte-identical) after the final verdict must not clear — no content check of any kind can see this, only lstat through the retained parent', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // AC-R1's proven-success fixture
    ...
    assert.doesNotMatch(reasonOf(result), REASON.ordinaryMismatch, 'nothing "mismatches" here — the bytes and the stamp agree perfectly. A mismatch reason would mean the refusal came from somewhere other than the identity binding');
...
// stamp. The R3 edge checks watch directory IDENTITIES, and `.claude/agents`
...
// the surface was verified. The latch is discharged, call N+1 stamps or accepts
...
// ===========================================================================
// AC-R28 — BINDING PROPERTY (d): THE STAMP'S OWN BOUND CONFIRMATION.
//
// Measured live-but-unpinned (0ac7be95 R5): stripping (d) left the suite green.
//
// The stamp is rewritten IN PLACE (`writeFileSync` truncates the SAME inode)
// after the final verdict, so the module's retained stamp descriptor is looking
// at bytes that no longer exist. Nothing else can see it: the stamp is not part
// of the (B) surface (AC-R1 clears with a stamp on disk and no entry attesting
...
//
// WHY IT MATTERS: the stamp on disk is what call N+1 compares the surface
...
// module clears against a stamp it never re-confirmed -> the
// `cleared`/`existsSync` assertions fire.
//
// SABOTAGE (one line): drop the stamp re-confirmation from
// `confirmBoundEvidence` (keep (a), (b), (c)) — cleared:true.
// ===========================================================================
test('AC-R28: the STAMP rewritten in place after the final verdict must not clear — the retained stamp descriptor must be re-confirmed against the verdict', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // AC-R1's proven-success fixture
    ...
    let hookCalls = 0;
    let stampIdBefore = null;
    let stampIdAfter = null;
    const result = await reconcileEnforcementTaint({
      cwd: dir,
      ...CONDUCTOR,
      _testHookInsideConfirm: () => {
        hookCalls += 1;
        stampIdBefore = identityOf(stampPath(dir));
        // Truncate-in-place on the same inode: the retained descriptor cannot
        // see this unless it is actually re-read.
        writeStamp(dir, [{ path: CODER_REL, sha256: 'deadbeef', at: NOW }]);
        stampIdAfter = identityOf(stampPath(dir));
      },
    });
    ...
    assert.equal(
      stampIdAfter,
      stampIdBefore,
      `FIXTURE CONTROL: this arm must be a TRUNCATE-IN-PLACE (same inode, new bytes), which is what isolates the CONTENT half of property (d) from the IDENTITY half AC-R47 pins. If the inode moved, this test is measuring AC-R47's guard (before=${stampIdBefore} after=${stampIdAfter})`
    );
    assert.notEqual(
      result.cleared,
      true,
    ...
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a post-verdict stamp rewrite');
    assert.match(reasonOf(result), REASON.stamp, `the refusal NAMES the stamp — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), REASON.changed, `the verdict is attributed to the stamp CHANGING since it was read, not to a fresh disagreement — reason: ${reasonOf(result)}`);
    assert.doesNotMatch(reasonOf(result), REASON.noLatch, 'the latch is present; a no-op verdict would mean the binding never ran');
  } finally {
    cleanup();
  }
...
// stamp, so no verification pass can see it either way — the guard under test
...
// root->`.sterling`, `.sterling`->`transient`. For an ABSENT root, retain
// EXPECTED ABSENCE and require `lstat` to still return ENOENT.
//
...
// match, the stamp is untouched, and the latch is removed although the LIVE
...
// descriptors — same detached directories, same name sets. (d) the stamp is
...
test('AC-R30: `.claude` RENAMED ASIDE and replaced after the final verdict must not clear — retained descriptors prove the OLD inode, not that the live path still reaches it', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  const build = () => {
    ...
    plantLatch(p.dir);
    writeStamp(p.dir, exactStampFor(p.dir)); // AC-R1's proven-success fixture
...
// ===========================================================================
// AC-R32 — THE MANIFEST SWAP: the identical trick on `.sterling`->`transient`,
// which 0ac7be95 R3 names explicitly ("The identical trick on
// `.sterling/transient` substitutes the manifest").
//
// THIS IS THE LAUNDERING SHAPE, and it is why the edge matters even though the
// stamp's own bytes are bound by property (d): (d) re-reads the RETAINED stamp
// DESCRIPTOR, which still holds the verified manifest inside the detached
// `transient` directory. The manifest the LIVE path now reaches — the one call
...
//
// ISOLATION: `.sterling`'s name set is unchanged (`transient` present before
// and after), the latch's own dev/ino is untouched, and the retained stamp fd
// still reads the good manifest — so (d), (c) and `removeLatch` are all blind.
//
// INJECTED AT SEAM 4b, LIKE AC-R30, so the FIRST-position edge check has
// already run and only the LAST-position re-check can refuse.
//
// EXPECTED FAILURE SHAPE: RED with `cleared:true` while the
// `.sterling`->`transient` edge is unretained or only re-confirmed at the first
// position. A `hookCalls` failure means the seam regressed instead.
//
// SABOTAGE (one line): drop the `.sterling`->`transient` edge
// re-confirmation — cleared:true, and call N+1 measures the surface against a
// manifest that was substituted after the verdict.
// ===========================================================================
test('AC-R32: `.sterling/transient` substituted after the final verdict must not clear — the LIVE manifest, not the retained descriptor, is what call N+1 will trust', { skip: BEHAVIOURAL_SKIP }, async () => {
  const build = () => {
    const p = makeGitProject();
    plantLatch(p.dir);
    writeStamp(p.dir, exactStampFor(p.dir)); // AC-R1's proven-success fixture
    const staging = makeStaging(p.dir);
    const newTransient = join(staging, 'transient-new');
    mkdirSync(newTransient, { recursive: true });
    writeFileSync(
      join(newTransient, 'enforcement-stamp.json'),
      JSON.stringify([{ path: CODER_REL, sha256: 'attacker-manifest-never-verified', at: NOW }])
    );
    return p;
  };
  ...
  try {
    ...
    const result = await reconcileEnforcementTaint({
      ...
      _testHookInsideConfirm: () => {
        hookCalls += 1;
        renameSync(join(dir, '.sterling', 'transient'), join(staging, 'transient-verified'));
        renameSync(newTransient, join(dir, '.sterling', 'transient'));
      },
    });
    ...
    assert.notEqual(
      result.cleared,
      true,
      `THE RULING (0ac7be95 R3): substituting the manifest DIRECTORY after the verdict launders the surface for call N+1 while the retained stamp descriptor still reads clean. reason: ${reasonOf(result)}`
    );
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a post-verdict manifest-directory substitution');
    assert.match(reasonOf(result), /transient|stamp/i, `the refusal NAMES the substituted manifest directory — reason: ${reasonOf(result)}`);
    ...
    assert.match(readFileSync(stampPath(dir), 'utf8'), /attacker-manifest-never-verified/, 'sanity: the substitution actually landed on the LIVE path');
  } finally {
    cleanup();
  }
});
...
// ===========================================================================
test('AC-R33 CONTROL: a project with NO `.claude` directory reconciles normally against a stamp attesting the remaining (B) surface', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    rmSync(join(dir, '.claude'), { recursive: true, force: true });
    plantLatch(dir);
    writeStamp(dir, [stampEntryFor(dir, CONFIG_REL)]); // the whole (B) surface, exactly
...
// (it never existed), so (a), (b) and (c) have nothing to check; the stamp is
...
test('AC-R34: `.claude` CREATED after the final verdict, in a project where it was absent at verification, must not clear — expected ABSENCE is evidence and must be re-checked', { skip: BEHAVIOURAL_SKIP }, async () => {
  const build = () => {
    ...
    plantLatch(p.dir);
    writeStamp(p.dir, [stampEntryFor(p.dir, CONFIG_REL)]); // identical to AC-R33's proven-clearing fixture
    const staging = makeStaging(p.dir);
    const newClaude = join(staging, 'claude-new');
    mkdirSync(join(newClaude, 'agents'), { recursive: true });
...
// which clears, so a stamp- or latch-family reason would mean the platform arm
...
test('AC-R36: on win32 reconciliation REFUSES and LEAVES THE LATCH PRESENT — it never degrades to a path-addressed re-read', { skip: GIT_SKIP }, async () => {
  ...
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // AC-R1's proven-success fixture: everything else is perfect
    ...
    assert.doesNotMatch(reasonOf(result), REASON.stamp, 'the stamp is exact here; a stamp-family reason would mean the platform arm never fired');
  } finally {
    cleanup();
  }
});
...
test('AC-R37: the pre-unlink bound confirmation is SYNCHRONOUS — an async callback would have its promise ignored and the unlink would proceed regardless', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // untouched fixture: the confirmation must succeed
...
// 32, files 256, dirents 4096, 8 MiB per file, 64 MiB total, stamp 8 MiB — and
...
//      stamp disagreement reports the wrong cause, and against a stamp built the
...
// win32 hashing path `readFileSync`s before enforcing 8 MiB; the stamp read
...
test('AC-R39: a (B) subtree deeper than the depth bound REFUSES, and says it was a bound', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampFor(dir)); // still EXACT — with the bound gone this fixture clears
    ...
    assert.match(
      reasonOf(result),
      LIMIT,
      `the verdict must be attributed to a BOUND. The stamp is EXACT here, so any other family would mean the depth limit never fired — reason: ${reasonOf(result)}`
    );
...
test('AC-R40: more walked (B) directories than the directory bound REFUSES, and says it was a bound', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampFor(dir)); // EXACT — empty directories contribute no (B) file
    ...
    assert.match(reasonOf(result), LIMIT, `the verdict must be attributed to a BOUND — the stamp is EXACT, so nothing else can explain a refusal — reason: ${reasonOf(result)}`);
...
test('AC-R41: more (B) files than the file/FD bound REFUSES even when every one of them is exactly attested', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampForWith(dir, extras)); // EXACT over all 303 files
    ...
    assert.doesNotMatch(
      reasonOf(result),
      REASON.incomplete,
      'every file here IS attested; an incomplete-stamp reason would mean the bound never fired and the manifest guard is carrying the verdict'
    );
  } finally {
    cleanup();
  }
...
// and the stamp remains exact — which is what makes `cleared !== true`
...
test('AC-R42: a walk whose cumulative dirent count exceeds the entry bound REFUSES with a bound-attributed reason, never as a manifest disagreement', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampFor(dir)); // the (B) set is still exactly the three attested paths
    ...
    assert.doesNotMatch(
      reasonOf(result),
      REASON.incomplete,
      'THE DISCRIMINATOR: an incomplete-stamp reason means the enumeration ran to completion and the manifest guard caught the fallout — i.e. no count bound fired at all, which is exactly the state R7 records'
    );
  } finally {
    cleanup();
  }
...
test('AC-R43: a (B) file larger than the per-file size bound REFUSES even when its hash is exactly attested', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampForWith(dir, [rel])); // EXACT — with the bound gone this clears
    ...
    assert.doesNotMatch(
      reasonOf(result),
      REASON.ordinaryMismatch,
      'THE TRUNCATION TRAP: a read that stops at the bound and hashes what it got produces a HASH MISMATCH against this fixture\'s whole-file stamp. That reports the wrong cause, and against a stamp built the same truncated way it would not refuse at all'
    );
  } finally {
    cleanup();
  }
...
test('AC-R44: a (B) surface whose TOTAL size exceeds the aggregate bound REFUSES even though every individual file is within the per-file bound', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  try {
    ...
    writeStamp(dir, exactStampForWith(dir, extras)); // EXACT over ~67.5 MiB
    ...
    assert.doesNotMatch(reasonOf(result), REASON.incomplete, 'every file here IS attested; an incomplete-stamp reason would mean the bound never fired');
...
// ===========================================================================
// AC-R45 — STAMP SIZE (8 MiB). The padding is TRAILING WHITESPACE, so the stamp
// still parses to EXACTLY the valid manifest AC-R1 clears with: with the stamp
...
// whole design of this fixture — a 9 MiB junk stamp would refuse as MALFORMED
// with the bound gone and pin nothing.
//
// SABOTAGE (one line): delete the stamp size check — cleared:true.
// ===========================================================================
test('AC-R45: a stamp file larger than the stamp size bound REFUSES, even though it parses to a perfectly valid exact manifest', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    // Valid JSON + 9 MiB of trailing whitespace: JSON.parse accepts it and
    // yields AC-R1's proven-clearing manifest.
    writeRawStamp(dir, JSON.stringify(exactStampFor(dir)) + ' '.repeat(9 * 1024 * 1024));

    const result = await reconcileEnforcementTaint({ cwd: dir, ...CONDUCTOR });
    assert.notEqual(result.cleared, true, `an over-large stamp must refuse rather than be read — reason: ${reasonOf(result)}`);
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a bound refusal');
    assert.match(reasonOf(result), LIMIT, `the verdict must be attributed to a BOUND — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), DIMENSION.size, `and must name the SIZE dimension — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), BINDING.stampBinding, `and must name the STAMP, not a (B) file — the (B) surface here is untouched and exactly three files — reason: ${reasonOf(result)}`);
    assert.doesNotMatch(
      reasonOf(result),
      REASON.malformed,
      'THE DISCRIMINATOR: this stamp is VALID JSON and parses to the exact manifest. A malformed reason would mean the module read all 9 MiB and then tripped over something else — i.e. the bound never fired'
    );
  } finally {
    cleanup();
  }
...
// stamp's IDENTITY arm (AC-R47 — AC-R28 pins only its CONTENT arm). Both are
...
test('AC-R46: BINDING (c) DISAPPEARANCE — a verified (B) member vanishing after the verdict must not clear', { skip: BEHAVIOURAL_SKIP }, async () => {
  const build = () => {
    ...
    writeStamp(p.dir, [...exactStampFor(p.dir), ...stampEntriesFor(p.dir, ['.claude/agents/second.md'])]);
    return p;
  };
...
// ===========================================================================
// AC-R47 — BINDING PROPERTY (d): THE STAMP'S IDENTITY ARM. The (b)-analogue for
// the stamp — a rename substitution with BYTE-IDENTICAL content, which the
...
// already covers the stamp's parent, so this is redundant." It is not.
// Per-edge retention covers DIRECTORY edges. A byte-identical stamp arriving by
// `rename` INSIDE `.sterling/transient` moves NO directory edge — `transient`
// keeps its identity, its name set is unchanged (`enforcement-stamp.json`
...
// stamp's own NAME through its pinned parent, compared against the retained
...
//
// EXPECTED FAILURE SHAPE (RED): without the stamp-name re-lstat the call CLEARS
// — the retained descriptor still holds the verified bytes and nothing else
// looks.
//
// SABOTAGE (one line): re-read the retained stamp descriptor but never
// re-`lstat` the stamp's name through its pinned parent — AC-R28 stays GREEN
// (its bytes changed, this one's did not), AC-R32 stays GREEN (no directory
// moved), and ONLY this test flips to cleared:true.
// ===========================================================================
test('AC-R47: BINDING (d) IDENTITY — the stamp RENAME-SUBSTITUTED with byte-identical content after the verdict must not clear', { skip: BEHAVIOURAL_SKIP }, async () => {
  const { dir, cleanup } = makeGitProject();
  try {
    plantLatch(dir);
    writeStamp(dir, exactStampFor(dir)); // AC-R1's proven-success fixture

    const stampBytes = readFileSync(stampPath(dir));
    let hookCalls = 0;
    let idBefore = null;
    let idAfter = null;
    let bytesAfter = null;
    ...
    const result = await reconcileEnforcementTaint({
      ...
      _testHookInsideConfirm: () => {
        hookCalls += 1;
        idBefore = identityOf(stampPath(dir));
        // Staged in `src/` — OUTSIDE both the (B) surface and
        // `.sterling/transient` — so no directory's name set is disturbed and
        // AC-R32's edge check cannot be the guard that carries this verdict.
        const staged = join(dir, 'src', 'staged-stamp.json');
        writeFileSync(staged, stampBytes); // byte-identical
        renameSync(staged, stampPath(dir)); // same name, NEW inode
        idAfter = identityOf(stampPath(dir));
        bytesAfter = readFileSync(stampPath(dir));
      },
    });
    ...
    assert.notEqual(idAfter, idBefore, `FIXTURE CONTROL: the stamp's dev/ino must actually change, or this pins nothing about identity (before=${idBefore} after=${idAfter})`);
    assert.deepEqual(
      bytesAfter,
      stampBytes,
      'FIXTURE CONTROL: the stamp bytes are IDENTICAL — that is what makes AC-R28\'s content re-read blind here, so a refusal is attributable to the identity arm alone'
    );
    assert.notEqual(
      result.cleared,
      true,
      `THE RULING: the authorising document at the stamp path is no longer the inode the verdict read. An attacker holding a write descriptor to the substituted inode rewrites it the moment the latch is gone. reason: ${reasonOf(result)}`
    );
    assert.match(reasonOf(result), BINDING.stampBinding, `the verdict must NAME the stamp — reason: ${reasonOf(result)}`);
    assert.match(reasonOf(result), BINDING.broken, `and must be attributed to its IDENTITY changing — reason: ${reasonOf(result)}`);
    assert.doesNotMatch(reasonOf(result), REASON.ordinaryMismatch, 'the stamp content is byte-identical; a mismatch reason would mean the refusal came from somewhere other than the identity binding');
    assert.doesNotMatch(reasonOf(result), REASON.noLatch, 'the latch is present; a no-op reason would mean the binding never ran');
    assert.equal(existsSync(latchPath(dir)), true, 'the latch survives a post-verdict stamp substitution');
  } finally {
    cleanup();
  }
});
...
// `.sterling/transient`, a leaf, or the stamp — all of which are EDGES, caught
...
//   * (d) the stamp — reachable through its retained descriptor, unchanged; and
...
test('AC-R49: the PROJECT ROOT (`cwd`) itself renamed aside and replaced by a byte-identical tree after the final verdict must not clear — only the retained anchor dev/ino can see this', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  const build = () => {
    ...
    // Copied AFTER the latch and stamp exist, so the replacement is a complete,
    // byte-identical project — latch, stamp, (B) surface and all. A replacement
    // MISSING the latch or the stamp would let a "no latch present" or "no
    // stamp" refusal carry the verdict, and this pin would be measuring a
    // sibling guard.
...
test('AC-R51: an unrecognised seam arriving INHERITED or NON-ENUMERABLE, and a recognised hook seam supplied as a NON-FUNCTION, each REFUSE BY NAME — a scan of enumerable own keys is not enough', { skip: BEHAVIOURAL_SKIP }, async () => {
  ...
  for (const arm of arms) {
    ...
    try {
      plantLatch(dir);
      writeStamp(dir, exactStampFor(dir)); // AC-R1's proven-clearing fixture
