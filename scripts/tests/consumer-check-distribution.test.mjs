...
// -- end-to-end: the GENERATED per-project launcher, not the two clone scripts
// directly (conductor follow-up, outside-model review) --------------------
//
// Every test above spawns check-record-citations.mjs / check-stale-claims.mjs
// straight out of THIS clone's scripts/ tree — which never exercises whatever
// init.mjs actually bakes into the consuming project (a baked path, a child
// node invocation, its own exit-code propagation). This test instead runs the
// artifact a consuming project's own tooling would actually invoke:
// <fixture root>/sterling-check.mjs, generated by init, executed with
// cwd == fixture root and no arguments — proving the launcher is genuinely
// runnable end-to-end from inside the consumer, not just its two ingredients.
//
// Exit-code TIERS (finding=1, capability_absent=2, env failure=3) are an
// in-flight implementation detail a coder is landing concurrently — this pin
// binds the SPEC only: clean == 0, and a seeded finding == nonzero AND names
// what it found. The seeded failure is an unresolved citation specifically
// because it needs no git-diff-base reasoning on the launcher's part (unlike
// a stale-claim finding, whose default --base the launcher computes
// internally and which this test deliberately does not assume the shape of).
test('sterling-check.mjs (the generated per-project launcher) exits 0 on a clean fixture, nonzero naming the finding on a seeded bad one', () => {
  const dir = buildConsumerFixture('sterling-consumer-launcher-');
  try {
    const goodId = seedDecision(dir);
    writeFileSync(join(dir, 'notes.md'), `See decision ${goodId.slice(0, 8)} for context.\n`);
    mkdirSync(join(dir, 'src'), { recursive: true });
    ...
    // a genuine follow-up commit, same as the CONTROL fixture above, so the
    // launcher's own diff-base logic (whatever it is) sees real history and
    // nothing in it carries an absence-claim.
    writeFileSync(join(dir, 'src', 'farm.mjs'), 'export function run() { return broadcast_trade_signal(1); }\n');
    assert.equal(git(dir, ['add', 'src/farm.mjs']).status, 0);
    assert.equal(git(dir, ['commit', '-m', 'wire the caller']).status, 0);

    const launcherPath = join(dir, 'sterling-check.mjs');
    assert.ok(existsSync(launcherPath), 'init must generate sterling-check.mjs at the consumer project root');

    const clean = spawnSync(process.execPath, [launcherPath], { encoding: 'utf8', cwd: dir, timeout: 120_000 });
    assert.equal(clean.status, 0, `the generated launcher must exit 0 on a clean consumer project: ${clean.stdout}${clean.stderr}`);

    // seed a bad citation — independent of whatever diff-base the launcher
    // picks for check-stale-claims, so this finding is guaranteed regardless.
    writeFileSync(
      join(dir, 'notes.md'),
      `See decision ${goodId.slice(0, 8)} for context.\n` +
        `See decision deadbeef for a rule that was never recorded here.\n` // not-a-citation: fixture id, deliberately unresolved
    ...
    assert.equal(git(dir, ['add', 'notes.md']).status, 0);
    assert.equal(git(dir, ['commit', '-m', 'introduce a bad citation']).status, 0);

    const bad = spawnSync(process.execPath, [launcherPath], { encoding: 'utf8', cwd: dir, timeout: 120_000 });
    assert.notEqual(bad.status, 0, `the generated launcher must fail on a seeded bad consumer project: ${bad.stdout}${bad.stderr}`);
    assert.match(bad.stdout + bad.stderr, /deadbeef/, 'the failure NAMES the finding, not just a bare nonzero exit');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
// SABOTAGE: hardcode sterling-check.mjs's generated body to always
// process.exit(0) (a launcher that never actually invokes the underlying
// checks) — the `bad` assertion (notEqual 0) fails immediately, caught; or
// have it exit nonzero without ever printing the finding (e.g. swallow child
// stdout/stderr) — the assert.match(/deadbeef/) fails, catching a silent
// failure that technically flags red but tells nobody why (P5).
