...
const CONVENTIONS = [
  ...
  '- ARTICLE APPLICATION IS DISPATCH-SHAPED: every knowledge_update/edit/append echoes the FULL updated record into the caller\'s context (~50KB on large articles). The conductor DRAFTS all reconcile text — the librarian never authors knowledge — then BATCHES the slice\'s drafted updates into ONE librarian dispatch (drafts + target ids + apply order) that returns only new record ids + versions and closes the reconcile_needed items its writes clear, running beside whatever the conductor is adjudicating. Hand-run store writes only for small authored creates (the echo IS the draft), a write needing live adjudication, or a single small-record touch (decision dac3d2c6).',
