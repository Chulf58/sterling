...
//       advising on the write that crosses it, not before. ONCE PER SESSION
//       (decision h21-downgraded-to-once-per-session, user-decided
//       2026-08-24): a session-scoped `advised` marker beside the tally gates
//       BOTH triggers, so the kind fires at most once per session; the tally
//       itself still counts every write. Both threshold fields are
...
//   (C) STREAK RESET — a Task/Agent dispatch resets the hand-work streak
//       counters but never resets the article-writes tally, which is a
//       whole-session count. ONCE PER SESSION (decision
//       h21-downgraded-to-once-per-session): the streak kind's `advised`
//       marker SURVIVES the episode reset, so a second episode in the same
//       session stays silent; only a new session_id re-arms either kind.
...
try {
  if (ARTICLE_WRITE_RE.test(toolName)) {
    ...
    // ONCE-PER-SESSION (decision h21-downgraded-to-once-per-session, user-
    // decided 2026-08-24): the article-write-bytes advisory KIND — whichever
    // of its two triggers (a single over-sized write, or the session's
    // cumulative bytes crossing session_bytes_advise) reaches it first — may
    // fire at most once per session. `advised` is the session-scoped marker,
    // held alongside the existing count/bytes tally; it is never cleared by
    // anything short of a session_id change. Thresholds are unchanged.
    // Legacy migration: a pre-upgrade state file has no `advised` field but
    // may carry `nagged: true` from the old session-crossing latch — treat
    // that as already-advised so a mid-session hook upgrade never re-fires.
    ...
    if (shouldEmit) {
      ...
      emit(
        `H21 article-write watch (once per session): ${sizeReason} — hand-run article write #${count} this session ` +
          `(decision dac3d2c6 — article application is librarian-shaped). Hand-run writes are for the three named ` +
          `exceptions only: (1) a small authored create, (2) a write needing live adjudication, (3) a single ` +
          `small-record touch. Bulkier article reconciles should batch through the librarian dispatch instead — it ` +
          `runs on a cheaper model in parallel, and the conductor's context window is the session's scarcest and ` +
  ...
  if (toolName === 'Task' || toolName === 'Agent') {
    // Streak reset (mechanism C) — a fresh episode can accumulate again later
    // in the same session, but the ONCE-PER-SESSION marker (`advised`) is
    // session-scoped, not episode-scoped, so it survives this reset: a
    // session that has already nagged once must not nag again from a later
    // episode. Article-writes is a whole-session tally and is deliberately
    // NOT touched here.
    const prior = readJsonSafe(streakPath, null);
    // Legacy migration: carry a pre-upgrade `nagged: true` into the surviving
    // `advised` marker across this reset, same rationale as kind A above.
  ...
  if (toolName === 'Read' || toolName === 'Grep' || toolName === 'Glob') {
    ...
    // Malformed-but-parseable state: a same-session object with an invalid
    // shape (e.g. read_paths not an array) must self-repair field-by-field
    // rather than throwing (which would fail-open and permanently suppress
    // this arm) or being discarded wholesale (which would lose valid fields).
    let streak;
    ...
    let ctx = null;
    // ONCE-PER-SESSION (decision h21-downgraded-to-once-per-session):
    // `streak.advised` is the session-scoped marker (untouched by the
    // Task/Agent episode reset above) — it gates in addition to the existing
    // per-episode `nagged` flag, so a later episode crossing the same
    // threshold again this session stays silent.
    if (!streak.nagged && !streak.advised && streakCount >= threshold) {
      streak.nagged = true;
      streak.advised = true;
      ctx =
        `H21 hand-work streak (once per session): ${streakCount} distinct hand-work action(s) (reads + searches) ` +
        `since the last dispatch — moment 3 of decision 677f1639: hand-work that needed only its CONCLUSION was a ` +
        `dispatch. Every hand-read lands file contents in the conductor's own context window — the session's ` +
        `scarcest and most expensive resource; a subagent (opus for judgment, sonnet for mechanical) returns only ` +
        `the conclusion. Delegate the remaining reads/searches.`;
    }
