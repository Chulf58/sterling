...
`);
  ...
  if (/[;&|`\n<>]|\$\(/.test(command)) {
    deny(
      `H14: shell control operators (chaining or redirection) are not allowed in agent commands: '${command}'. This is the single-command allowlist design: chaining/redirection are rejected BEFORE prefix matching, because an operator can extend an otherwise-allowed command into one that is not.`
    );
  }
