...
function latestUsage(path) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (usage2 && typeof usage2.input_tokens === "number") {
...
if (!usage && reason === "format_unparseable") {
  ...
  if (store) {
    try {
      store.recordCheckSkipped("context-watch", reason ?? "format_unparseable", store.getRun()?.id, (/* @__PURE__ */ new Date()).toISOString());
    } finally {
      store.close();
    }
  }
