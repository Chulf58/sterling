...
var SterlingStore = class {
  ...
  casTransition(observed, next) {
    ...
    const res = this.db.prepare("UPDATE runs SET machine_state = ?, pending_exit = NULL, body = ?, updated_at = ? WHERE id = ? AND machine_state = ?").run(run2.machine_state, JSON.stringify(run2), (/* @__PURE__ */ new Date()).toISOString(), run2.id, observed);
    if (res.changes === 0) {
...
try {
  ...
  if (input.hook_event_name === "PostToolUse") {
    const fillsPath = run ? join2(input.cwd, ".sterling", "runs", run.id, "h6-fills.jsonl") : join2(input.cwd, ".sterling", "transient", "h6-fills.jsonl");
    mkdirSync2(dirname2(fillsPath), { recursive: true });
    appendFileSync(
      fillsPath,
