  ...
  applyInPlace(op, id, buildPatch, opts, internal = {}) {
    ...
    this.tx(() => {
      ...
      const res = this.db.prepare(`UPDATE records SET version = ?, status = ?, lifecycle = ?, freshness = ?, superseded_by = ?,
             updated_at = ?, derived_unconfirmed = ?, body = ? WHERE id = ? AND version = ?`).run(nextVersion, _SterlingStore.derivedStatus(identity.lifecycle, freshness), identity.lifecycle, freshness, supersededBy, stored.updated_at ?? now2, stored.derived_unconfirmed ? 1 : 0, JSON.stringify(stored), id, identity.version);
      if (res.changes === 0) {
