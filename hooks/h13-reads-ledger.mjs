...
var SterlingStore = class _SterlingStore {
  ...
  casTransitionMerge(observed, runId2, mutate, attempts = 5) {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      let changes = 0;
      this.tx(() => {
        changes = Number(this.db.prepare("UPDATE runs SET machine_state = ?, pending_exit = ?, body = ?, updated_at = ? WHERE id = ? AND body = ? AND machine_state = ? AND pending_exit IS ?").run(next.machine_state, tail, JSON.stringify(next), (/* @__PURE__ */ new Date()).toISOString(), runId2, row.body, observed, row.pending_exit).changes);
      });
      if (changes === 1)
        return next;
    }
  ...
  updateRunOptimistic(runId2, mutate, attempts = 5) {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      let changes = 0;
      this.tx(() => {
        changes = Number(this.db.prepare("UPDATE runs SET body = ?, updated_at = ? WHERE id = ? AND body = ?").run(JSON.stringify(next), (/* @__PURE__ */ new Date()).toISOString(), runId2, row.body).changes);
      });
      if (changes === 1)
        return next;
    }
