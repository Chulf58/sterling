...
var SterlingStore = class {
  ...
  casTransitionMerge(observed, runId2, mutate, attempts = 5) {
    for (let i = 0; i < attempts; i++) {
      ...
      if (row.machine_state !== observed) {
        throw new Error(`CAS rejected: run '${runId2}' is not in observed state '${observed}' \u2014 stale caller; re-read run_state, never re-apply (\xA75.2)`);
      }
  ...
  writeHandoff(runId2, input2, at) {
    ...
    if (!this.db.prepare("SELECT 1 FROM runs WHERE id = ?").get(runId2)) {
      throw new Error(`writeHandoff: no run '${runId2}'`);
    }
    this.db.prepare("INSERT INTO handoffs (run_id, phase_id, agent_role, body, created_at) VALUES (?, ?, ?, ?, ?)").run(runId2, handoff.phase_id, handoff.agent_role, JSON.stringify(handoff), at);
    return handoff;
  }
  readHandoffs(runId2, filter = {}) {
    const rows = filter.phase_id ? this.db.prepare("SELECT body FROM handoffs WHERE run_id = ? AND phase_id = ? ORDER BY created_at").all(runId2, filter.phase_id) : this.db.prepare("SELECT body FROM handoffs WHERE run_id = ? ORDER BY created_at").all(runId2);
   ...
   */
  updateRunOptimistic(runId2, mutate, attempts = 5) {
    for (let i = 0; i < attempts; i++) {
      const row = this.db.prepare("SELECT body FROM runs WHERE id = ?").get(runId2);
      if (!row)
        throw new Error(`updateRunOptimistic: no run '${runId2}'`);
      ...
      const res = this.db.prepare("UPDATE runs SET body = ?, updated_at = ? WHERE id = ? AND body = ?").run(JSON.stringify(next), (/* @__PURE__ */ new Date()).toISOString(), runId2, row.body);
      if (res.changes === 1)
        return next;
    }
    throw new Error(`updateRunOptimistic: lost the optimistic race ${attempts}x for run '${runId2}' (P5: failing loudly)`);
  }
  /** H6 context warns + run_escalate land here (§6). */
  appendRunEscalation(runId2, entry) {
    this.updateRunOptimistic(runId2, (run) => ({ ...run, escalations: [...run.escalations, entry] }));
  }
  /** H7 pipeline mark (§6): article reconciliation due at completion; idempotent. */
  appendRunReconcileNeeded(runId2, articleId) {
    this.updateRunOptimistic(runId2, (run) => (run.reconcile_needed ?? []).includes(articleId) ? run : { ...run, reconcile_needed: [...run.reconcile_needed ?? [], articleId] });
  }
  /**
   * Mid-run scope amendment (brief mid-run-scope-amendment, decision 8e6f9491):
   ...
   */
  appendRunScopeAmendment(runId2, amendment) {
    this.updateRunOptimistic(runId2, (run) => (run.scope_amendments ?? []).some((a) => a.path === amendment.path) ? run : { ...run, scope_amendments: [...run.scope_amendments ?? [], amendment] });
  }
  /**
   * Per-phase reviewer mandatory set (decision 628c4b7f, run r-d630, phase 1 — AC1):
   ...
   */
  setRunReviewMandatory(runId2, phaseId, items) {
    this.updateRunOptimistic(runId2, (run) => {
  ...
  incrementDispatchCount(runId2, agentType) {
    const next = this.updateRunOptimistic(runId2, (run) => ({
      ...run,
   ...
   */
  disposeRunRows(runId2, summaries) {
    const run = this.getRun(runId2);
    if (!run)
      throw new Error(`disposeRunRows: no run '${runId2}'`);
    if (run.machine_state !== "completing") {
      throw new Error(`disposeRunRows: run '${runId2}' is '${run.machine_state}', not 'completing' \u2014 disposal is the completion sequence only`);
    }
    const next = runRecordSchema.parse({ ...run, machine_state: "awaiting_merge_gate", summaries });
    this.tx(() => {
      const res = this.db.prepare("UPDATE runs SET machine_state = ?, pending_exit = NULL, body = ?, updated_at = ? WHERE id = ? AND machine_state = ?").run(next.machine_state, JSON.stringify(next), (/* @__PURE__ */ new Date()).toISOString(), runId2, "completing");
      if (res.changes === 0)
        throw new Error(`disposeRunRows: CAS rejected for run '${runId2}' (stale caller)`);
  ...
  recordCheckSkipped(check, reason, runId2, at) {
    this.db.prepare("INSERT INTO check_skipped (run_id, check_name, reason, at) VALUES (?, ?, ?, ?)").run(runId2 ?? null, check, reason, at);
    if (!runId2) {
  ...
  listCheckSkipped(runId2) {
    return runId2 ? this.db.prepare("SELECT run_id, check_name, reason, at FROM check_skipped WHERE run_id = ? ORDER BY seq").all(runId2) : this.db.prepare("SELECT run_id, check_name, reason, at FROM check_skipped ORDER BY seq").all();
  }
  // -------------------------------------------------------------------------
