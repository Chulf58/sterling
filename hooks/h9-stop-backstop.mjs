...
function verifyPromotionConditions({ store, config, run }) {
  ...
  if (article) {
    const tracedOn = /* @__PURE__ */ new Map();
    ...
    for (const ac of brief.acceptance_criteria) {
      if (!tracedOn.has(ac.ac_id)) {
        refuse(
          "ac_untraced",
          `AC '${ac.ac_id}' has no live_test_refs entry with a non-empty test_paths on any of the ${articles.length} article(s) historied to this brief (checked: ${checkedNames})`
        );
      }
    }
    for (const a of articles) {
