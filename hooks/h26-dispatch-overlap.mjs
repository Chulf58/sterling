...
function buildResourceAdvisory(contested) {
  ...
  return `H26 RESOURCE OVERLAP ADVISORY \u2014 this dispatch's brief claims exclusive resource(s) ${resourceList}, already held by live in-flight dispatch(es): ${holderList}. This is warn-only, never a block (decision 6de73875-75b5-4182-8c1c-ca4841c993fa). It may repeat on further dispatches while the holding dispatch stays live, since the prompt extraction only approximates territory. Remedy: coordinate with the holder before proceeding, or drop the resource claim.`;
}
var input;
try {
  input = readStdin();
