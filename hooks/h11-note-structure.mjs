...
var SterlingStore = class {
  ...
  appendNoteDerived(noteId2, derivedIds) {
    const note = this.get(noteId2);
    if (!note || note.type !== "note")
      throw new Error(`appendNoteDerived: '${noteId2}' is not a note`);
    const updated = validateRecord({ ...note, derived: [.../* @__PURE__ */ new Set([...note.derived, ...derivedIds])] });
    this.db.prepare("UPDATE records SET body = ? WHERE id = ?").run(JSON.stringify(updated), noteId2);
  }
  /**
...
var noteText = input.tool_input?.fields?.raw_text;
var noteId;
try {
...
function skipLoud(reason) {
  store.recordCheckSkipped("note-structuring-h11", reason, store.getRun()?.id, (/* @__PURE__ */ new Date()).toISOString());
  store.close();
