...
try {
  ...
  if (current + 1 > cap) {
    ...
    deny(
      `H8: dispatch cap exceeded \u2014 '${agentType}' has been dispatched ${current}x this run (cap ${cap}). This run is looping; escalate to the human instead of spawning again (\xA75.1). This denial repeats by design for every further dispatch of that type in this run \u2014 it is the terminal per-run loop cap (\xA75.1), not a retryable hook failure.`
    );
  }
