...
try {
  try {
    ...
    process.stderr.write(`H19 notice drain: ledger prune failed: ${e && e.message || e}
`);
  }
  ...
  process.stdout.write(JSON.stringify({ hookSpecificOutput: { hookEventName: "UserPromptSubmit", additionalContext: texts.join("\n\n") } }), (err) => {
    if (err) {
      process.stderr.write(`H19 notice drain: write failed: ${err && err.message || err}
`);
      process.exit(0);
    }
    for (const file of files) rmSync2(file, { force: true, recursive: file === legacy });
    process.exit(0);
  });
} catch (e) {
  warnNonBlocking(`H19 notice drain failed: ${e && e.message || e}`);
}
