    ...
    if (mode === "inject") {
      ...
      const assemble = (pathM2) => {
        ...
        if (!unowned) {
          ...
          porch = porchBudget > 0 ? renderPorch(porchHeaderLine([rel]), freshHazards, freshOwners, porchBudget, {
        ...
        const deferredHazards = hazardParts(shownHazards2.filter((hazard) => deferredHazardIds.has(hazard.id)), { fileKeys: [rel] });
        const parts = porch.text ? [...migrationNoticeParts, ...porch.parts, ...deferredHazards, ...decisionParts, ...ownerParts, ...suspectParts] : [
