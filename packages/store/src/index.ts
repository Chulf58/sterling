...
export class SterlingStore {
   ...
   */
  private ftsTermExpr(term: string): string {
    return term.endsWith('*') && term.length > 1
      ? `"${term.slice(0, -1).replace(/"/g, '""')}"*`
      : `"${term.replace(/"/g, '""')}"`;
  }
   ...
   */
  private distinctTermExprs(terms: string[]): string[] {
  ...
  query(opts: QueryOptions = {}): DurableRecord[] {
    ...
    if (opts.rank_terms !== undefined) {
      const terms = rankTerms.parse(opts.rank_terms);
      if (terms.length) {
        // COVERAGE RANKING (decision pull-ranking-at-scale-order-of-work-
        // coverage-before-columns-no-narrowing-ladder, step 2): order the
        // WHOLE eligible set — before the cap — by how many DISTINCT
        // rank_terms expressions a row matches (FTS semantics: a prefix or a
        // term's own quoting is ONE expression, never a substring count),
        // THEN bm25, so a record flooding one generic term cannot outrank
        // one that matches every term (mechanism 1, research_finding
        // why-dome-farmer-pull-cases-miss-mechanisms-september-2026).
        //
        // `match` (ftsMatchExpr) and the coverage CTE arms below both derive
        // from distinctTermExprs() — the ONE dedupe point (see its doc
        // comment: exact-string, NOT case-folded, and load-bearing for bm25
        // itself, not just for coverage counting). countAboveScore shares
        // the same ftsMatchExpr call, so it can never threshold a different
        // (inflated) score than the one query() orders by.
        const match = this.ftsMatchExpr(terms, opts.match_all);
        const distinctTermMatches = this.distinctTermExprs(terms);

        // With ONE distinct term, or match_all set, coverage is UNIFORM
        // across every eligible row (OR: the sole term; AND: every term, by
        // construction of the match filter) — order degenerates to bm25
        // alone, so the CTE below is skipped entirely: cheaper, and it also
        // makes that case byte-identical to the pre-coverage query shape
        // (plus the new r.id ASC final key).
        ...
        // SET-ORIENTED coverage: one MATCH scan per DISTINCT term producing
        // rowids (`hits`), aggregated into a per-row coverage count (`cov`),
        // then joined to the normal eligible query — O(terms) FTS scans
        // total instead of O(rows × terms) correlated lookups. An INNER join
        // on `cov` cannot drop an eligible row: under OR every eligible row
        // matches at least one term, hence appears in at least one `hits`
        // arm (match_all is already routed above, so this branch is OR-only).
        const hitsArms = distinctTermMatches.map(() => 'SELECT rowid FROM records_fts WHERE records_fts MATCH ?').join(' UNION ALL ');
        const sql = `WITH hits(rowid) AS (${hitsArms}),
        ...
        // Parameter order follows the SQL TEXT left-to-right: the CTE's `?`
        // placeholders (one per distinct term) sit before the WHERE clause's
        // (baseFilter's `params`), which sit before the eligibility `match`
        // and the LIMIT `cap`.
        const rows = this.db.prepare(sql).all(...distinctTermMatches, ...params, match, cap) as { body: string; scope: string }[];
        return this.withDerivedReliedByAll(SterlingStore.decodeLiveRecords('query', rows));
      }
    }
