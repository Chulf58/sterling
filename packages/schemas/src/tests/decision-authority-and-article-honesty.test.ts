...
test('B1 (CONTROL, positive): current_ac[].untestable_because {reason, blocking_record_id} parses and round-trips (board 6a8507f8)', () => {
  ...
  const article = articleBase([
    {
      ...
      verifiable_at: 'final',
      untestable_because: { reason: 'no harness can drive a real browser download — ruled out by decision 2176748e-72f6-4cfc-a790-7fd67c7ee6aa', blocking_record_id: blockingId }, // not-a-citation: fixture id
    },
  ]);
  ...
  assert.equal(parsed!.current_ac[0].untestable_because!.reason, 'no harness can drive a real browser download — ruled out by decision 2176748e-72f6-4cfc-a790-7fd67c7ee6aa'); // not-a-citation: fixture id
