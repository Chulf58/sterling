...
// ===========================================================================
// TDD toggle row (decision foreign_752caf98, tdd-and-mutation-toggles-in-
// system-tab) — SPEC-ONLY, written against the decision's WIRING clause
// ("the TUI System tab gains two rows following the exact
// sparring_partner.enabled row pattern") without reading state.ts/main.ts.
// Mirrors packages/tui/src/tests/sparring-partner.test.ts's structure and
// CLEAN-RED discipline exactly, scoped to the toggle-only half of that
// pattern (tdd carries no model field, so there is no free-text row here —
// only the ON/OFF row sparringRows[0] modeled).
//
// The sibling mutation_verification toggle row this file used to also pin
// was REMOVED entirely (decision
...
// Every test below that mixed the two rows is rewritten to pin the tdd row
// alone; the pure mutation-only tests (ON/OFF rendering, SPACE-toggle,
// cross-row independence) are DROPPED rather than rewritten, since their
...
//   • buildSystemTab's returned view carries a NEW separate array,
//     `tddRows: SystemRow[]`, alongside the existing `rows` and
//     `sparringRows` (both of those stay untouched).
//   • tddRows carries EXACTLY ONE row — the toggle row — mirroring
//     sparringRows[0]'s shape (id/lines), never the model-row half of that
//     pattern (tdd has no model field).
//   • the injected AgentRosterSnapshot (interface slice, extended for this
//     slice) carries a new field: `tdd: { enabled: boolean }` (the live
//     config value).
//   • cursor indices continue past the two sparringRows entries: the tdd
//     row = configModels-key-count + 2. DOWN past the tdd row clamps (it is
//     the last row on the tab).
//   • the toggle effect: { type: 'tdd_toggle', enabled: boolean } — the
//     FLIPPED value, not the pre-toggle value, exactly like sparring_toggle.
//   • ENTER and SPACE both toggle; no picker/selector opens for the row.
//   • while a config.models picker is open, tddRows is empty — the same
//     rule item 7 already pins for sparringRows in sparring-partner.test.ts.
//
...
/** Extended for this slice with tdd (see the CONTRACT comment above) — the
 *  new field the System tab's toggle row reads. sparringPartner/codexWired
 *  travel too since baseSnapshot must stay a valid AgentRosterSnapshot for
 *  the already-landed sparring-partner slice. */
...
// Existence-asserts before indexing, so a not-yet-implemented tddRows fails
// on a clean AssertionError, never a TypeError from reading `.lines` off
// `undefined` (CLEAN-RED discipline, mirrors the typeof-buildSystemTab
// guards used throughout this file and its sibling sparring-partner.test.ts).
...
// ===========================================================================
// Item 1 — tddRows separate from rows and sparringRows; carries exactly one
// row
// ===========================================================================

test('toggles 1: buildSystemTab exposes tddRows as a separate single-entry array; rows/sparringRows untouched', () => {
...
// ===========================================================================
// Item 2 — ON/OFF rendering
// ===========================================================================

test('toggles 2: the tdd row shows ON when tdd.enabled=true and OFF when false', () => {
  assert.strictEqual(typeof buildSystemTab, 'function', 'buildSystemTab must be exported');
  const on = buildSystemTab!(baseSnapshot({ tdd: { enabled: true } }), st({ tab: SYS_TAB }), 80);
  ...
  const off = buildSystemTab!(baseSnapshot({ tdd: { enabled: false } }), st({ tab: SYS_TAB }), 80);
  assert.match(tddRowText(off), /\bOFF\b/, 'tdd.enabled:false renders OFF on the tdd row');
});

// ===========================================================================
// Item 3 — cursor traversal past sparringRows onto the tdd row
// ===========================================================================

test('toggles 3: UP/DOWN traverse past sparringRows onto the tdd row, and clamp at the bottom', () => {
  ...
  try {
    ...
    // by sparring-partner.test.ts) — this slice's row continues from there.
...
// ===========================================================================
// Item 4 — ENTER/SPACE on the row: the flipped-value toggle effect, no
// picker opens
// ===========================================================================
...
test('toggles 4: SPACE on the tdd row emits tdd_toggle with the flipped value (false -> true); no picker opens', () => {
  const { store, cleanup } = storeFixture();
  try {
    const snap = baseSnapshot({ tdd: { enabled: false } });
    const numKeys = Object.keys(snap.configModels).length;
    const r = SR.reduce(store, st({ tab: SYS_TAB, cursor: numKeys + 2 }), key('SPACE'), undefined, undefined, snap);
    const toggle = findTddToggle(r.effects);
    assert.ok(toggle, 'SPACE on the tdd row emits a tdd_toggle effect');
...
// ===========================================================================
// Item 5 — while a config.models picker is open, tddRows is empty (mirrors
// the frozen sparringRows rule, sparring-partner.test.ts item 7)
// ===========================================================================

test('toggles 5: while a config.models picker is open, tddRows is empty (but still present as an array)', () => {
  const { store, cleanup } = storeFixture();
  try {
    assert.strictEqual(typeof buildSystemTab, 'function', 'buildSystemTab must be exported');
    const snap = baseSnapshot();
...
test('toggles 6: buildDashboardState honors ui.scroll on the System tab — the tdd row (last row) stays inside a bounded viewport after DOWN-walking, not hardcoded to 0', async () => {
  const { store, cleanup } = storeFixture();
  try {
    assert.strictEqual(typeof stateMod.buildDashboardState, 'function', 'buildDashboardState must be exported');
    const snap = baseSnapshot();
    const numKeys = Object.keys(snap.configModels).length;
    const maxBodyLines = 2; // total SYS_TAB rows (numKeys config rows + 2 sparring + 1 tdd) exceed this
    const vp = { maxBodyLines, width: 80 };

    // DOWN-walk from the top all the way onto the tdd row (cursor numKeys+2,
    // the last row on the tab), letting the reducer's revealAt push
    // ui.scroll as the cursor passes out of the small viewport — the exact
    // mechanism state.test.ts pins for the card tabs.
    ...
    assert.ok(
      (dash.scroll ?? 0) > 0,
      'the selected tdd row must sit inside the bounded viewport, which requires a nonzero scroll',
    );
  } finally {
    cleanup();
  }
