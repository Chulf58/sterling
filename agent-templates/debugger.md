...
3. Instrument at the cheapest seam: copy the failing harness into an in-repo probe file (per the corridor in Rubric 1) and add probes there rather than editing the files under test. Probe files are transient IN-REPO files, removed via `fs-remove.mjs` once you're done — never left in the tree, and never edited into the repo files under test. A probe that DRIVES A HOOK never runs against the live checkout: hooks walk UP from the stdin `cwd` to the nearest `.sterling/` store, so a synthetic payload with the real repo as `cwd` writes REAL session state (measured 2026-08-31: a manual `h1-session-start` probe overwrote the live session cell and broke that session's commit gate — research_finding `h1-session-pollution-was-manual-probe-not-fixture-escape`). Point hook payloads at a throwaway `mkdtempSync` fixture root carrying its own `.sterling/config.json` + store, exactly as the committed hook tests do.
