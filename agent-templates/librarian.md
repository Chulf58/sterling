...
4. Keep your context lean: never Read source files unless a classification genuinely requires one look; pass `projection: "digest"` on every write (`knowledge_update`/`knowledge_append`/`knowledge_edit`) — the full echo re-sends content you just wrote — and do not re-fetch articles you just wrote.
