...
- **Notes are the user's surface.** Conductor knowledge is born structured (`decision` / `anti_pattern` / `research_finding` / article reconciliation) — never parked in a note; a `note` you create only relays a user statement verbatim, never a task in disguise. Misfiled or spent notes leave via `note_remove`, on the user's word. **THE THREE WRITE SURFACES ARE NOT INTERCHANGEABLE — do not use one as another.** Work you intend to do goes on the **BOARD** (`/sterling:todo`, or `board_add` with `source:'user'`): that is the todo/task list, it is the only surface for wanted-but-not-done work, and an item leaves it solely through the artifact-write that fulfils it (`board_remove`) — never by being called done in conversation. The **MAINTENANCE QUEUE** (`board_query source:'system'`, drained by `/sterling:drain`) is **NOT** a todo board: it is mechanism-detected debt, every item carrying a registered `system_reason` (`reconcile_needed`, `capture_owed`, `stale_research`, `article_missing`, …), minted by the event that detected the debt and removed by the artifact that closes it (P4). Never hand-park a task there to remember it, and never treat a drain as optional tidying. A **NOTE** is neither: it relays what the user said, verbatim, and nothing else. **WHEN TO DRAIN: at session start if the queue is deep, and before taking new work.** H1 says so when the queue crosses `config.maintenance_queue.deep_threshold` and names the lanes; silence means nothing is owed. Expect a deep queue to be mostly already-done work that was never closed — verify each item against HEAD, and close an already-paid one with `board_remove` and **no** `knowledge_update` (a version bump claiming a reconcile that added nothing is itself drift). A queue growing faster than it closes is its own finding: a skipped drain, or a hook over-firing.
