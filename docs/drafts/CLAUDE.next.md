...
- **Mechanism inventories do not live in THIS file — the store is the census.** Never copy in what a self-describing surface answers: record shapes (`knowledge_schema`), the hook roster (`hooks.json`), the anti-pattern set (`knowledge_query`). A summary of the store is a second copy of it and rots accordingly. This file carries durable conventions and the incidents justifying them; the repo layout and invariants are the deliberate exceptions.
