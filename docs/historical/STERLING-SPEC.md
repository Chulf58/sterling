...
| TUI | **TypeScript** + **terminal-kit** | **Evidence-resolved (reversing the planned Ink choice):** the daily-driver Forge observer is terminal-kit, and its native mouse support (click-to-expand, right-click, wheel) is the proven UX property — base Ink cannot deliver it (addon ecosystem is alpha). **Structure required:** a pure, fully-tested state layer (Forge's own `buildDashboardState` pattern) with terminal-kit as a thin render layer — testability lives in the state layer, never the renderer |
...
sterling/
  ...
  tui/                             # TypeScript TUI: pure tested state layer + thin terminal-kit renderer
...
**Out-of-band drift is caught at read time (use-proven gap, adjudicated 2026-06-11):** H7 covers Sterling-governed touches; edits made outside Sterling fire no hook, so `knowledge_query` checks every served feature article against disk — any owned file (`files[].path`) with mtime newer than the article's `updated_at`, **or missing from disk** (deletion is drift), flags the result `verify_before_use` and enqueues a **deduplicated** `reconcile_needed` maintenance item naming the drifted file (same reason + `feature_link` dedup as H7 — one drain surface for "this article needs truing up" regardless of trigger; a hundred stale reads make one queue entry). Reconciliation clears the flag mechanically: `knowledge_update` bumps `updated_at` past the file mtimes. The flag is computed at read, never persisted (P4). Scope: the file-key relation only — no propagation through `dependencies`; indirect effects surface when the flagged article is reconciled (the reconciler consults `relied_by` as content work).
...
**3.2.5 `reference_material`** — `title, kind (pdf|url|doc), location, summary, source_date, capture_date, basis (codebase|platform|external, default codebase)`. Large, stable, loaded on demand; never bulk-injected. **Repo-located docs join the reconcile economy (use-proven gap):** when `kind: doc` and `location` is repo-relative, the location doubles as a `file_key` — H7's touch register marks the record `reconcile_needed` when Sterling-governed work edits the file; reconciliation = refresh summary + `source_date` (the summary is the FTS retrieval surface; a stale map silently degrades retrieval — P5). **Out-of-band edits are caught at read time:** for repo-located docs, `knowledge_query` compares file mtime against `source_date` — file newer → flagged `verify_before_use` **and a deduplicated maintenance item (`refresh_reference`) is enqueued** (a hundred stale reads make one queue entry; the queue's H1 counts and human-decides drain give staleness one actionable surface instead of ambient flag noise). External locations (pdf|url) carry no file_keys; `basis` + stale-at-read covers their rot.

**3.2.6 `note`** — `raw_text` (immutable), `captured_at`, `capture_source (tui|command|conductor)`, `derived[]` (links to extractions). On capture, a Haiku structuring pass extracts candidate decisions/anti-patterns/article-annotations as records flagged `derived_unconfirmed` (marked lower-trust), each citing the note. **Mechanism (build-resolved): H11 is a command hook invoking headless Haiku (`claude -p --model haiku`) and writing through the validated store path** — prompt-type hooks are read-only and cannot write records; CLI or extraction failure records `check_skipped {note-structuring-h11}`, never silent. Raw text is never modified; extraction is re-runnable. **Retrieval default: `derived_unconfirmed` records are EXCLUDED from `knowledge_query` unless the caller passes `include_unconfirmed: true`** — pipeline agents never receive unconfirmed material silently (an unconfirmed wrong "decision" in staged context steers work and competes for cap slots; the damage is silent — P5). The conductor opts in at intake/planning; **any unconfirmed record actually relied on for a brief or design is confirmed-or-killed on the spot** (confirming strips the flag, making it default-visible thereafter; use is the confirmation event — the unconfirmed pool drains exactly when the knowledge matters, with a human present). **Notes are the user's surface (adjudicated 2026-06-12):** `capture_source: conductor` means the conductor relaying a user-origin statement verbatim — conductor-generated knowledge is born structured (`decision` / `anti_pattern` / `research_finding` / article reconciliation), never parked in a note. **Removal: `note_remove(id)`** deletes a note outright, the same P4 hard removal todos get (raw-text immutability governs edits, not deletion); inbound `cites` links and derived extractions survive as independent records, exactly as `fulfills` links survive `board_remove`.
...
**There is no `done` status.** Done = the record is **removed**, atomically, by the same hook that writes the durable artifact representing its completion (P4). History lives in the artifact (which `fulfills`-links the todo's text), not on the board. The board only ever contains open work. **Queue drain log (adjudicated 2026-06-12):** removing a *system-source* todo additionally appends `{drained_at, system_reason, text, file_keys}` to a **capped** drain log in the store (audit projection, precedent `check_skipped`; cap 50, pruned in the same transaction — completed items never build up). User-source todos are never logged. The record itself is still hard-deleted — the log answers "was X handled?", it is not a done-pile.
...
Platform constraint: only the session can dispatch subagents (the Agent tool, formerly Task). Therefore:
...
- **The hands** are the conductor: the dying agent's last acts are `handoff_write` + **`agent_exit(signal)` — an MCP call, never prose**: zod validates the signal at the server (an invalid signal is rejected in-band, so the agent sees the error and corrects itself), and the exit lands on the run record. **Exit routing (run-proven, r-0001): abnormal exits go to `run_signal` immediately from any position. Normal `complete` is phase-scoped** — a non-terminal step's `complete` (e.g. the test-writer's) is consumed by the conductor as the next step of the 8.1 sequence, recorded on the run record via same-state CAS (clearing the pending-exit slot; the audit trail holds); `run_signal` receives `complete` only at the phase boundary — "every exit → run_signal" predates phases containing multiple normal exits. Every dispatch passes through the conductor's hands. Task returned with no exit recorded → `agent-died{empty_output}`; a crash maps to `agent-died{crash}` — the conductor never silently retries, never invents a signal, never routes around the brain. *(With the exit in-band, `malformed_exit` survives only as the defense-in-depth case: garbage reaching `run_signal` itself.)*
...
|---|---|---|---|---|---|---|
| H1 | conventions + banner | SessionStart | command | no | project | inject conventions (anti-speculation, no-false-action, canonical names); print banner art to stderr (swappable art slot, fixed-width ≤40 cols — fits the 35% split pane; suppression is env-only, `STERLING_NO_BANNER=1` — a SessionStart hook sees no CLI flags or pipe state, so `--no-banner`/piped detection have no mechanical home; adjudicated 2026-06-12) + **board/maintenance counts** via systemMessage ("7 todos · 3 maintenance items pending") — the queue is event-drained and otherwise invisible; this is its visibility pressure |
...
| H8 | dispatch cap | PreToolUse `Agent` (the spawn tool — formerly `Task`; **probe-verified**: spawning surfaces as a blockable PreToolUse call with `tool_input.subagent_type`, despite docs suggesting only non-blocking SubagentStart) | command | YES | project | increment per-agent-type run counter; over cap → deny + escalate |
...
| H10 | direct-path capture check + review | Stop | command → prompt | soft | project | artifact-produced-but-no-capture → prompt conductor to capture inline; still missing → maintenance queue (`capture_owed`). Code-touching diff → run reviewer-selection script and dispatch exactly what it returns; test-touching → test-integrity vs **git HEAD**. **Article demand (adjudicated 2026-06-11):** touched files no feature_article owns, at threshold (config `article_demand.min_unowned_files`, default 3) **or any new unowned file** (vs git HEAD; no-git degrades loud to threshold-only, `check_skipped`) → the same nag demands the OWNING ARTICLE inline (knowledge is freshest before the session ends); still missing at session end → `article_missing` maintenance item carrying the unowned file list (deduped by file-key overlap). General capture does NOT satisfy the article demand — the §12 accretion point made enforceable, not hoped for |
...
| H12 | wiring + zero-consumer check | run-completion script (part of final completeness, not a per-edit hook) | command | gates | project | static: new exports referenced only by tests = built-but-not-wired → block unless **dormancy is declared on the owning article** (state `dormant` + `state_reason` + `wiring_todo_id`, §3.2.3 — the checked artifact; the brief may *anticipate* dormancy in its design section, but emergent dormancy is declared at capture) → then write `wire_in_dormant` todo. Dynamic wiring is covered by the end-to-end AC tests |
...
| H15 | store write-path guard | PreToolUse `Bash\|PowerShell` | command | **YES exit-2** | project | **adjudicated 2026-06-12 (incident-driven):** any shell command referencing the project store (`.sterling/` path or `sterling.db`) is DENIED — reads included — unless it invokes a sanctioned script/launcher (config `store_guard.allow_scripts`; defaults `scripts/dispose-run.mjs`, `scripts/init.mjs`, `scripts/consume-exit.mjs`, `sterling-tui.mjs`). The store is read and written through the §10 tool surface only; the deny message NAMES the tools and the rule: a server lagging the code means restart the session, never write around the surface. The guard model: deterministic deny at the seam + a message that teaches the right path; deny patterns grow incident-by-incident via config, never speculatively |
...
| `note_remove(id)` | hard removal for notes — the user-surface mirror of `board_remove` (3.2.6, adjudicated 2026-06-12): misfiled or spent notes leave the Notes surface outright; refuses non-note records |
...
- **Queue tab (adjudicated 2026-06-12):** the maintenance queue (same todo store, `source: system`) gets its own tab — pending items as selectable cards in the **upper half** (clipped at a fixed divider with a "… N more pending" line), the last 15 **drain-log** entries in the **lower half** (dim; **not selectable** — they are log lines, not records). Completed lines read `HH:mm <action> · <target>` (`MM-dd HH:mm` when older than today): the action verb derives from `system_reason` via the registry's `DRAIN_VERBS` map (draining means the fulfilling artifact was written — reconcile_needed→updated, article_missing→created, capture_owed→captured, …; totality-checked so a new lane cannot render blank), the target is the quoted name in the item text, else the first file key `(+N)`. **Ordering is by drain sequence (newest first) — the displayed stamp is cosmetic and never a sort key.** The divider stays fixed at half height even when the queue is empty. The TUI can never dismiss a queue item — selection delegates the drain to the conductor; the artifact-write is what removes it (P4).
...
8. **Pipeline mode** (8.1) incl. branch manager (run branch / per-phase commits / merge gate / discard paths) and both completeness checkers. **Resolved (build-verified): plan-mode-shaped.** Native plan mode is read-only with per-call MCP prompts (P1 ceremony) and cannot deliver the approved plan as the brief object — the planning skill's read-only discipline + the store brief as single authority; native plan mode is not load-bearing.
9. **Conductor-direct + debug + cleanup** (8.2–8.4) incl. H10 and reviewer-selection script.
10. **TUI** (11). **Resolved (evidence-superseded): terminal-kit** — the ink-terminal verify found only an alpha; base Ink 7 was briefly chosen, then reversed on evidence: the user's daily-driver UX (native mouse — click, right-click, wheel) is terminal-kit's, and base Ink can't provide it. Structure: pure tested state layer + thin terminal-kit renderer; selection, bundling, and non-TTY behaviors are renderer-agnostic and unchanged.
