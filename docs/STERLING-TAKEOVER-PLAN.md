...
Slices 3b, 6 and 7 and the small bug batch are closed. **EXIT AND RELAUNCH before anything else** — commits `674a50c`, `4288bf9`, `1ce375d` and `a37db67` changed MCP-server, store and hook code the running session never loaded (on 2026-09-21 the old server was still minting one reconcile item per file). Then, in order: prove `4288bf9` live; ~~own the ruling corpus~~ (DONE, `f287581`, which ALSO changed hook and MCP-server sources and rebuilt their bundles — one more reason to relaunch); ~~board hygiene~~ (DONE 2026-09-21: Slice 2 `eaa9dd5c` rewritten to its acceptance session only, Slice 1 `c6180ba0` rewritten to uv/Blender MCP + SpaceExplorer + backup rehearsal with the domain-store copy turned into a do-NOT, Astra follow-ups `5e032781` removed with its one remainder folded into Slice 1); drain the maintenance queue (`/sterling:drain` — expect the sweep's ~200 touched files to have minted reconcile items that are already paid: a comment token swap owes no article change); the article-coverage pass (`0dca9d42`, `f72e5982`, `b0e8325a`); pull quality at scale (`58eaafa5`) with the Bash push gap (`c0be9034`). The **Dome Farmer acceptance session** — a real `/sterling:update` on Dome Farmer and SpaceExplorer — comes when the user is at the machine and has ruled on SpaceExplorer prune-or-re-init.
