...
- This clone is `machine_role: authoring` but `store_authority: secondary`: none of the 408 record ids cited by `rulings.md` (generated 2026-09-08) resolve in this project store or its domain stores — the ruling corpus the code cites lives in another machine's store (decision 'Citation and projection authority is per-store', `a446753c`). USER-RULED 2026-09-21: no import — this store starts over as the corpus (decision `this-store-starts-over-as-the-ruling-corpus-no-import`, `7e7d8eda`). Follow-on slice boarded as `19c7bb59`: design what the 912 dead citations become, sweep by script, regenerate the projections, and only THEN flip `store_authority` to `primary` — flipping first fails `npm run check` and makes `/sterling:update` unfinishable. The configured `backup_path` listed no files on 2026-09-21.
...
Slices 3b, 6 and 7 and the small bug batch are closed. **EXIT AND RELAUNCH before anything else** — commits `674a50c`, `4288bf9`, `1ce375d` and `a37db67` changed MCP-server, store and hook code the running session never loaded (on 2026-09-21 the old server was still minting one reconcile item per file). Then, in order: prove `4288bf9` live; own the ruling corpus (board `19c7bb59`); board hygiene — rewrite the stale Slice 2 (`eaa9dd5c`), Astra follow-ups (`5e032781`) and Slice 1 (`c6180ba0`) items and drain the maintenance queue; the article-coverage pass (`0dca9d42`, `f72e5982`, `b0e8325a`); pull quality at scale (`58eaafa5`) with the Bash push gap (`c0be9034`). The **Dome Farmer acceptance session** — a real `/sterling:update` on Dome Farmer and SpaceExplorer — comes when the user is at the machine and has ruled on SpaceExplorer prune-or-re-init.
