...
**Ensure-manifest semantics (re-run safe — adjudicated):** `/sterling:init` is an *ensure* operation, not a one-shot. Every manifest item is **verified individually**: absent → **created**; present and matching the expected generated content → **skipped, reported as matching**; present but differing (hand-edited, tuned, or foreign) → **left untouched and reported** — init never overwrites content it cannot prove it generated. **Refusal is reserved for destructive actions only** (e.g., a file occupying a path the manifest requires as a directory); "already initialized" is *not* a refusal, and a pre-existing CLAUDE.md is never clobbered — init completes around it and reports that the conductor contract must be merged by hand (the merge is a human/conductor act, never init's). **Every manifest artifact is individually regenerable:** delete it and re-run init — on re-run the declarations (stack tags, toolchains, backup choice, domains) are read back from the recorded config, so no flags are required; flags passed on re-run that contradict the recorded config are reported, never silently applied (the config may be tuned — editing it is the owner's act). The agent-installation item follows the §13 sync semantics (installed / refreshed / up-to-date / refuse-on-local-modification); the restart instruction is emitted whenever any agent was installed or refreshed. Mixed-state stores (store present, config absent, or vice versa) are the normal case for this semantics, not an error.
